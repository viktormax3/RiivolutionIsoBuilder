...
public static partial class RiivolutionPatchPlanner
{
    public static RiivolutionPlan CreateDefaultPlan(RiivolutionDocument document, string gameId)
    {
        ...
        {
            var choiceIndex = option.DefaultChoice > 0 ? option.DefaultChoice - 1 : 0;
        ...
        if (activePatches.Count == 0 && document.Patches.Count == 1)
        {
    ...
    public static XDocument CreateDolPatchXml(RiivolutionPlan plan, string? mainDol)
    {
        ...
        foreach (var memory in plan.ActivePatches.SelectMany(patch => patch.MemoryPatches))
        {
            ...
            {
                element.SetAttributeValue("valuefile", Resolve(memory.ValueFile, plan.Parameters).Replace('\\', '/'));
            }
